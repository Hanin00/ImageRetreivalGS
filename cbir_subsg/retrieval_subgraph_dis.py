...
from utils import utils_dis as utils
...
def load_dataset_temp(args,F0Dict):
    ...
    with open("result/vid08_1215_sceneg_4005_subg_12926_dis.pkl", "wb") as fw:
...
def load_dataset(): #동일 조건 하에서
    ...
    # with open("result/vid04_1217_from_db.pkl", "rb") as fr:
    # with open("result/vid08_1217_from_db.pkl", "rb") as fr:
        datas = pickle.load(fr)
...
def feature_extract(args):
    ...
    with torch.no_grad():
        ...
        for idx, queryG  in enumerate(querys): #i = 쿼리 그래프의 서브 그래프 하나.
            ...
            # print("@@@")
            # print(sorted_items[:20])  #이미지 영상_프레임_서브그래프 번호 정보     
    ...
    with open("result/1217/vid08_1217_from_db_dis.pkl", "wb") as fw:
    # with open("result/result_graphs_dis_video02.pkl", "wb") as fw:
